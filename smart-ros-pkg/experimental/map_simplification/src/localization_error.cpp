...
localization_error::localization_error()
{
	...
	lat_error_pub_ = nh.advertise<sensor_msgs::PointCloud>("lat_error", 10);
	lon_error_pub_ = nh.advertise<sensor_msgs::PointCloud>("lon_error", 10);
...
void localization_error::updateErrorSummary(map_simplification::error& er)
{
	...
	e_sum.count++;
	e_sum.ave_lat += er.lat_error;
	e_sum.ave_lon += er.lon_error;
	e_sum.ave_abs += er.abs_error;
...
void localization_error::syncCallback(const geometry_msgs::PoseWithCovarianceStampedConstPtr amcl, const nav_msgs::OdometryConstPtr ground_truth)
{
	...
	xy_error.z = error_msg.lon_error;
	lon_error.points.push_back(xy_error);
	xy_error.z = error_msg.lat_error;
