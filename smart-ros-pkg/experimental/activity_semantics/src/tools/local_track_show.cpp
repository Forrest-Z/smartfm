...
namespace golfcar_semantics{
	...
	void local_track_show::show_update(double x, double y, CvPoint & prev_point, CvScalar bgr_color)
	{
		...
		cvCircle( local_trajectory_img_, current_point, 1, bgr_color, 2);
	...
	void local_track_show::clear_image()
	{
		cvZero(local_trajectory_img_);
		draw_backGND();
	}

	void local_track_show::save_image(const char* image_name)
	{
		cvSaveImage(image_name, local_trajectory_img_);
	}
