...
struct reference_pose
{
	...
	reference_pose():valid(false),yaw(0){}
	...
	{
	    header = ps.header;
	    yaw = getYawFromQuadMsg(ps.pose.orientation);
	    yaw = convertToPi(yaw);
	    valid = true;
	    return yaw;
	}
...
class LaserVehicle
{
	...
	void referPoseCB(geometry_msgs::PoseStamped::ConstPtr ps_)
	{
		if(target_frame_ == "base_link")
		{
          rp_.ps = *ps_;
          rp_.update_value();
		}
    ...
    void scanCallback(sensor_msgs::LaserScanConstPtr scan)
    {
        ...
        result = simpleEuclideanExtraction(laser_range_pt);
    ...
    bool simpleEuclideanExtraction(LaserRangePoint &lrp)
    {
        ...
        ret =  selectVehicle(segmented_pts, laser_cloud.header);
    ...
    bool selectVehicle(vector<pcl::PointCloud<pcl::PointXYZ> > &segmented_pcl, std_msgs::Header header)
    {
        ...
        if(target_set_)
        {
            ...
            if(rp_.valid == true)
            {
            	...
            	double final_yaw;
            	double temp_yaw = getYawFromQuadMsg(final_pose.pose.orientation);
            	temp_yaw = convertToPi(temp_yaw);

            	if((temp_time - rp_.header.stamp)>ros::Duration(0.5))
            	{
            		final_yaw =  temp_yaw ;
            	}
            	else if(fabs(temp_yaw - rp_.yaw)>0.1)
				{
					final_yaw = rp_.yaw;
            		//final_yaw = temp_yaw;
				}
            	else if(fabs(temp_yaw - rp_.yaw)<0.02)
            	{
            		final_yaw = temp_yaw;
            	}
            	else if((temp_time - rp_.header.stamp)>ros::Duration(0.3))
            	{
            		final_yaw = 0.01 * temp_yaw + 0.99 * rp_.yaw;
            		cout<<"changing 0.5"<<endl;
            	}
            	else if((temp_time - rp_.header.stamp)>ros::Duration(0.1))
            	{
            		final_yaw = 0.01 * temp_yaw + 0.99 * rp_.yaw;
            		cout<<"changing 0.3"<<endl;
            	}
            	else
            	{
            		final_yaw = 0.001 * temp_yaw + 0.999 * rp_.yaw;
            		cout<<"changing 0.1"<<endl;
            	}
            	//final_yaw =  rp_.yaw;
            	final_pose.header.stamp = ros::Time::now();
            	//final_yaw = filter_yaw_->filter(final_pose.header.stamp.toSec(),final_yaw);

            	final_pose.pose.orientation = getQuadMsgFromYaw(final_yaw);


            }
            ret = this->publishVehicle(final_pose);
        }
        else
        {
            ...
            if(size_filtered_pcl.points.size()>0)
            {
                ...
                nnt_ = new NearestNeighborTracking(nh_,vehicle_pose);
                target_set_ = true;
            }
        }
    ...
    bool publishVehicle(geometry_msgs::PoseStamped vehicle_pose)
    {
        static int vehicle_pose_num_ = 0;
        bool ret = false;
        if(vehicle_pose.header.seq  != 0)
        {
            ret = true;
        }
        if(ret == true)
        {
           ...
           pa_.header = vehicle_pose.header;
           pa_.poses.push_back(vehicle_pose.pose);
			if(vehicle_pose_num_ >=500)
			{
			   pa_.poses.erase(pa_.poses.begin());
			}
			else
			{
				vehicle_pose_num_ ++;
...
public:
    LaserVehicle(): nh_(new ros::NodeHandle), tf_(new tf::TransformListener), tf_broadcaster_(new tf::TransformBroadcaster), target_set_(false)
    {
        ...
        ref_pose_sub_ = nh_->subscribe("/robot_1/ref_tf",10,&LaserVehicle::referPoseCB,this);
