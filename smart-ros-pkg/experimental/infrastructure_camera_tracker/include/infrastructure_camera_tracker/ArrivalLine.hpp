...
class ArrivalLine
{
  ...
  public:
    ...
    ArrivalLine(cv::Scalar line_color)
    {
      id = id_count++;
      color = line_color;
    };
    ...
    const bool getIntersection( const cv::Point2f& position,
                                const cv::Point2f& velocity,
                                cv::Point2f& intersection_point,
                                float& time_to_arrival,
                                float& x);

    const cv::Point2f getIntersectionPoint(const cv::Point3f& line)
    {
    ...
    const cv::Point2f getIntersectionPoint(const cv::Point2f& pt_1, const cv::Point2f& pt_2)
    {
    ...
    const void print(std::ostream& out, std::string pre)
    {
    ...
    const bool validateIntersectionPoint(const cv::Point2f& point)
    {
