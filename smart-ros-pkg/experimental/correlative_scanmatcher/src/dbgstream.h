...
public:
    /// constructor.
    dbgbuf() : tee(0), enable_output_(false)  { };

    /// Destructor.  No magic here.
    ~dbgbuf();

    void enable_output() {enable_output_ = true;};
    void disable_output() {enable_output_ = false;};
...
public:
    ...
    explicit dbgstream() :
		...
		{ }
    void enable_output() {sbuf.enable_output();};
       void disable_output() {sbuf.disable_output();};
