...
public:
	...
	void getInputPoints(vector<T> raster_pt_input, vector<T> normal_input)
	{
		...
		norm_image_ = cv::Mat::zeros( (int) map_size.y, (int) map_size.x  , CV_32FC1);


		fmutil::Stopwatch sw_draw("Confirming rastermap time");

		stringstream distance, norm_file;
		distance<<"rastered_map_"<<res_<<"_"<< range_covariance_<<".png";
		...
		if(use_normal_)
		{
			...
			{
				cv::Point pt =imageCoordinate(raster_pt[i]);
				double angular_norm = atan2(normal_input[i].y, normal_input[i].x);
				cv::circle(norm_image_, pt, temp_r, cv::Scalar(angular_norm), -1);
			}
		}
		sw_draw.end(false);
