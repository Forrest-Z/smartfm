...
static const std::string pc_topic_ = "pc";
class AmclNode
{
public:
	AmclNode();
...
private:
	...
	dynamic_reconfigure::Server<amcl_pc::AMCLConfig> *dsrv_;
	amcl_pc::AMCLConfig default_config_;
	...
	void reconfigureCB(amcl_pc::AMCLConfig &config, uint32_t level);
        		...
        		first_reconfigure_call_(true)
{
	...
	else if(tmp_model_type == "pc_likelihood_field")
			laser_model_type_ = PC_MODEL_LIKELIHOOD_FIELD;
	else
	{
	...
	pc_scan_filter_ =
				new tf::MessageFilter<sensor_msgs::PointCloud2>(*pc_scan_sub_,
						*tf_,
						odom_frame_id_,
						100);
	pc_scan_filter_->registerCallback(boost::bind(&AmclNode::pcReceived,
				this, _1));
	...
	dsrv_ = new dynamic_reconfigure::Server<amcl_pc::AMCLConfig>(ros::NodeHandle("~"));
	dynamic_reconfigure::Server<amcl_pc::AMCLConfig>::CallbackType cb = boost::bind(&AmclNode::reconfigureCB, this, _1, _2);
...
void AmclNode::reconfigureCB(amcl_pc::AMCLConfig &config, uint32_t level)
{
	...
	if(config.laser_model_type == "beam")
		...
		laser_model_type_ = LASER_MODEL_LIKELIHOOD_FIELD;
	else if(config.laser_model_type == "pc_likelihood_field")
			laser_model_type_ = PC_MODEL_LIKELIHOOD_FIELD;
	...
	else if(laser_model_type_ == PC_MODEL_LIKELIHOOD_FIELD)
	{
...
AmclNode::pcReceived(const sensor_msgs::PointCloud2ConstPtr& pc_scan)
{
	...
	resampleParticles(force_publication, resampled, odom_pose, pc_scan->header);
}
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
	...
	initializeSensor(laser_scan->header, laser_index, odom_pose, pose, force_publication);
	// If the robot has moved, update the filter
	bool resampled = false;
	if(lasers_update_[laser_index])
	{
	...
	resampleParticles(force_publication, resampled, odom_pose, laser_scan->header);*/
	tf::Stamped<tf::Pose> odom_pose; pf_vector_t pose;
	...
	if(latest_tf_valid_)
	{
		...
		// everybody happy.
		ros::Time transform_expiration = (header.stamp +
...
void
AmclNode::initializeSensor(std_msgs::Header header, int &laser_index, tf::Stamped<tf::Pose> &odom_pose, pf_vector_t &pose,
		bool &force_publication)
{
	...
	if(frame_to_laser_.find(header.frame_id) == frame_to_laser_.end())
	{
		ROS_DEBUG("Setting up laser %d (frame_id=%s)\n", (int)frame_to_laser_.size(), header.frame_id.c_str());
		...
		frame_to_laser_[header.frame_id] = laser_index;
	} else {
		// we have the laser pose, retrieve laser index
		laser_index = frame_to_laser_[header.frame_id];
	}
...
void
AmclNode::resampleParticles(bool force_publication, bool resampled, tf::Stamped<tf::Pose>& odom_pose, std_msgs::Header header)
{
