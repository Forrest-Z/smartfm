...
int main(int argc, char **argv)
{
	...
	ros::init(argc, argv, "golfcar_sampler");
	ros::NodeHandle n_;
	ros::Publisher chatter_pub = n_.advertise<golfcar_halsampler::odo>("golfcar_sampler", 1000);
	signal(SIGINT,quit);
	...
	for ( n = 0 ; n < fifo->num_pins ; n++ ) {
	    ...
	    {
	    	if(n==0)
	    	odometer.pose = buf[n].f;
	    	if(n==1)
	    	odometer.vel = buf[n].f;
	    	if(n==2)
	    	odometer.steering_angle = buf[n].f;
		}
		break;
	    case HAL_BIT:
		if(n==3){
