 ...
 * 			-D_FORTIFY_SOURCE: Provides compile-time best-practices errors for certain libc functions, and provides run-time checks of buffer lengths and memory regions
 * 			-ffast-math: control optimisation
 * 			-mhard-float: Generate output containing floating point instructions
 * 			-DRTAI: no info
 * 			and bla, bla, bla....
 * 		After trials, turns out only require this: gcc -I/usr/include/emc2 -DULAPI -o streamer_gc3 streamer_gc.c -lemchal 
 * */
...
void golfcar_break(golfcar_lowlevel::breakpedal bp)
{
  ...
  break_ = bp.angle;
  sprintf(incoming,"%lf %lf %lf\n",speed_, steering_, break_);//joy_.axes[1]*joy_.axes[2],-joy_.axes[0]*180.0);//
  fifo_out(incoming);
}
...
int main(int argc, char **argv)
{
	...
	/* initialize ros as listener*/
	ros::init(argc, argv, "golfcar_halstreamer");
	ros::NodeHandle n;
	ros::Subscriber speed_sub = n.subscribe("golfcar_speed", 1, golfcar_speed);
	ros::Subscriber break_sub = n.subscribe("golfcar_brake", 1, golfcar_break);
	ros::Subscriber steering_sub = n.subscribe("golfcar_steering", 1, golfcar_steering);
	puts("Start spining");
	ros::spin();
	/* Send data 
