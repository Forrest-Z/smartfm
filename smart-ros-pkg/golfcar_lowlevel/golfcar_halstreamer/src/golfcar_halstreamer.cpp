...
void golfcar_vel(golfcar_lowlevel::vel vel_)
{
  ROS_INFO("Speed=%lf, Angle=%lf", vel_.speed, vel_.angle);//-joy_.axes[0]*180.0);//(int)velocity.angle);
  char incoming[100];
  int break_;
  if(vel_.speed == 0) break_ = -0;
  else break_= -120;
  sprintf(incoming,"%lf %lf %d\n",vel_.speed, vel_.angle, break_);//joy_.axes[1]*joy_.axes[2],-joy_.axes[0]*180.0);//
  fifo_out(incoming);
}
...
int main(int argc, char **argv)
{
	...
	ros::NodeHandle n;
	ros::Subscriber sub = n.subscribe("golfcar_vel", 1000, golfcar_vel);
	puts("Start spining");
	ros::spin();
	/* Send data 
