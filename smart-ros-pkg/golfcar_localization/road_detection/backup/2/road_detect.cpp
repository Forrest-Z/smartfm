...
namespace road_detection{
	...
	road_detect::road_detect()
	{
		...
		//change parameters to get better performances; many methods can be applied here;
		/*
	...
	{	
		...
		//*****see curb_track****
		//sick_to_baselink_.transformPointCloud("base_link", laser_cloud_, cloud_baselink_);
	...
	void road_detect::RoadSelection()
	{
	    ...
	    std::vector<unsigned int> temp_C1C2;     //store pairs of points, with each pair of points corresponding to one road candidates.
	    	    
		//ROS_INFO("2------Curb candidate numbers %d",curb_candidate_PIDs_.size());
		for(unsigned int i=0; i<(curb_candidate_PIDs_.size()-1);i++)     //pay attention to "i<(curb_candidate_PIDs_.size()-1)";
		{
	...
	void road_detect::CurbSideLine()
	{
		...
		if((left_line_begin_PID!=0)&&(left_line_end_PID!=0))
		{
			...
			bool temp_left_height=(temp_delt_z>0.05&&temp_delt_z<0.3)||(temp_delt_z<-0.05&&temp_delt_z>-0.3);
	    ...
	    if((right_line_begin_PID!=0)&&(right_line_end_PID!=0))
		{
			...
			bool temp_right_height=(temp_delt_z>0.05&&temp_delt_z<0.3)||(temp_delt_z<-0.05&&temp_delt_z>-0.3);
