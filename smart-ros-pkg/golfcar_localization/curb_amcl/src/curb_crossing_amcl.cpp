...
pf_vector_t
CurbCrossingNode::uniformPoseGenerator(void* arg)
{
  map_t* map = (map_t*)arg;
...
CurbCrossingNode::curbReceived (const sensor_msgs::PointCloud::ConstPtr& cloud_in)
{
	...
	else
	{
		...
		if(updateMeas)
		{
		    ...
		    crossing_->UpdateSensor(pf_, (AMCLSensorData*) LeftCroData_);
		    crossing_->UpdateSensor(pf_, (AMCLSensorData*) LeftCroData_);
		    
		    RightCroData_->sensor = crossing_;
		    crossing_->UpdateSensor(pf_, (AMCLSensorData*) RightCroData_);
		    crossing_->UpdateSensor(pf_, (AMCLSensorData*) RightCroData_);
