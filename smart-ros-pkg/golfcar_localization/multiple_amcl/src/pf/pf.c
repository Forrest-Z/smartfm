...
void pf_update_sensor(pf_t *pf, pf_sensor_model_fn_t sensor_fn, void *sensor_data, bool *UseFlag)
{
  int i;
  pf_sample_set_t *set;
  pf_sample_set_t *workset;
  pf_sample_t *sample;
  pf_sample_t *worksample;
  double total;

  pf_sample_set_t workspace;
  
  workset = &workspace;
  
  set = pf->sets + pf->current_set;

  workset->sample_count = set->sample_count;
  ...
  for (i = 0; i < workset->sample_count; i++)
  {
    worksample = workset->samples + i;
	 sample = set->samples + i;
    worksample->weight = sample->weight;
  ...
  else
  {
	*UseFlag=true;
	  for (i = 0; i < workset->sample_count; i++)
	  {
	    worksample = workset->samples + i;
		 sample = set->samples + i;
		 sample->weight = worksample->weight;
	  }
  }
