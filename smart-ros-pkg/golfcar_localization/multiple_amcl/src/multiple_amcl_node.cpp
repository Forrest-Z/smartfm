/* NUS Golf Cart
 * MultiAmclNode is based on the original Amcl node, which is designed to support multiple sensory modalities;
 * 20120801
 */
...
class MultiAmclNode
{
  ...
  private:
    ...
    // the 3 most fundamental functions for multiple_amcl;
    void laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan);                                
    void curbReceived (const sensor_msgs::PointCloud::ConstPtr& cloud_in);
	void markerReceived (const vision_lane_detection::markers_info::ConstPtr& makers_in);
    ...
    message_filters::Subscriber<vision_lane_detection::markers_info>* marker_sub_;
    tf::MessageFilter<vision_lane_detection::markers_info>* visionMarker_filter_;
	...
	AMCLMarker* visionMarker_;
	AMCLMarkerData* visionMarkerData_;
	
    map_t* requestMap(const std::string& map_service);
    
    //the following function is to initialize marker map by reading text files;
    //later may also write it in a service-call way like "requestMap";
	marker_map* requestMarkerMap(const std::string& markerMap_path);
    ...
    //add on 20120106, use other sensors to eliminate noise in curb measurements;
...
{
  ros::init(argc, argv, "multiple_amcl");
  ros::NodeHandle nh;
	...
	private_nh_("~")
{
  ...
  private_nh_.param("marker_d_thresh_", marker_d_thresh_, 0.2);	 	
  private_nh_.param("marker_a_thresh_", marker_a_thresh_, M_PI/6);   
  ...
  std::string markerMap_path ="/home/baoxing/workspace/data_and_model/visionMarker.map";
  
  private_nh_.param("x_sigma", x_sigma, 0.5);
  private_nh_.param("y_sigma", y_sigma, 0.5);
  private_nh_.param("thetha_sigma", thetha_sigma, M_PI/180.0*10.0);  
  private_nh_.param("dis_outlier", dis_outlier, 1.0);  
  
  curb_map_   = requestMap(curb_svc);
  planar_map_ = requestMap(planar_svc);
  marker_map_ = requestMarkerMap(markerMap_path);
  
  // Create the particle filter
  pf_ = pf_alloc(min_particles, max_particles,
                 alpha_slow, alpha_fast,
                 (pf_init_model_fn_t)MultiAmclNode::uniformPoseGenerator,
                 (void *)curb_map_);
  pf_->pop_err = pf_err;
  pf_->pop_z = pf_z;
  ...
  //visionMarker_;
  visionMarker_ = new AMCLMarker(marker_map_);
  visionMarkerData_ = new AMCLMarkerData();
  ROS_INFO("Initializing vision marker simple model");
  visionMarker_->SetMarkerModel(x_sigma, y_sigma, thetha_sigma, dis_outlier);
  ROS_INFO("Done initializing vision marker simple model");
  ...
  global_loc_srv_ = nh_.advertiseService("global_localization", &MultiAmclNode::globalLocalizationCallback, this);
  ...
  curb_filter_->registerCallback(boost::bind(&MultiAmclNode::curbReceived, this, _1));
  ...
  laser_scan_filter_->registerCallback(boost::bind(&MultiAmclNode::laserReceived,this, _1));
  
  marker_sub_ = new message_filters::Subscriber<vision_lane_detection::markers_info>(nh_, "markers_info", 10);   
  visionMarker_filter_ = new tf::MessageFilter<vision_lane_detection::markers_info>(*marker_sub_, *tf_, odom_frame_id_, 100);
  visionMarker_filter_->registerCallback(boost::bind(&MultiAmclNode::markerReceived, this, _1));
  ...
  initial_pose_filter_->registerCallback(boost::bind(&MultiAmclNode::initialPoseReceived, this, _1));

  initial_pose_sub_old_ = nh_.subscribe("initialpose", 2, &MultiAmclNode::initialPoseReceivedOld, this);
  invalid_curb_pub_ = nh_.advertise<sensor_msgs::PointCloud>("invalid_curb", 2);
  
  //otherSource_sub_ = nh_.subscribe("sickldmrs/assembled", 10, &MultiAmclNode::otherSourceCallback, this); 
  ...
  tf_filter_->registerCallback(boost::bind(&MultiAmclNode::otherSourceCallback, this, _1));
...
map_t* MultiAmclNode::requestMap(const std::string& map_service)
{
	map_t* map = map_alloc();
	ROS_ASSERT(map);
...
marker_map* MultiAmclNode::requestMarkerMap(const std::string& markerMap_path)
{
	marker_map* map = marker_map_alloc();
	ROS_ASSERT(map);
	
	//open file to read;
	FILE *fp_restore;
	const char *restore_filename = markerMap_path.c_str();
	fp_restore = fopen(restore_filename,"r");
	if(fp_restore==NULL)
	{
		fprintf(stderr,"can't open marker map file %s\n", restore_filename);
		exit(1);
	}
	cout<<"marker map file opened"<<endl;
	
	int max_index=0;
	int idx; 
	while(fscanf(fp_restore,"%d %*d %*f %*f %*f\n",&idx) == 1) max_index = std::max(idx,max_index);
	rewind(fp_restore);
	int total_number = max_index+1; 	//because feature index begins from 0, rather than 1;
	cout<<"map features retrieved, number: "<<total_number<<endl;
	
	map->markers = (marker_feature *)malloc((total_number)* sizeof(marker_feature));
	map->marker_count = (size_t)max_index + 1;

	marker_feature feature_tmp;
	while(fscanf(fp_restore,"%d %d %lf %lf %lf\n",&idx, &feature_tmp.marker_type, 
		  &feature_tmp.maker_pose.v[0], &feature_tmp.maker_pose.v[1], 
		  &feature_tmp.maker_pose.v[2])==5)
	{
		if(idx<=max_index)
		{
			map->markers[idx] = feature_tmp;
		}
	}
	fclose(fp_restore);
	cout<<"marker map initialized; file closed"<<endl;
	return map;
}
...
bool MultiAmclNode::getOdomPose(tf::Stamped<tf::Pose>& odom_pose, double& x, double& y, double& yaw, double& pitch,
							  const ros::Time& t, const std::string& f)
{
...
pf_vector_t MultiAmclNode::uniformPoseGenerator(void* arg)
{
	//map_t* map = (map_t*)arg;
...
bool MultiAmclNode::globalLocalizationCallback(std_srvs::Empty::Request& req,std_srvs::Empty::Response& res)
{
	pf_mutex_.lock();
	
	ROS_INFO("Initializing with uniform distribution");
	pf_init_model(pf_, (pf_init_model_fn_t)MultiAmclNode::uniformPoseGenerator,(void *)curb_map_);
	pf_init_ = false;
	curb_init_ =false;
	laser_init_=false;
	marker_init_ = false;
...
void MultiAmclNode::otherSourceCallback (const sensor_msgs::LaserScan::ConstPtr& scan_in)
{
...
void MultiAmclNode::curbReceived (const sensor_msgs::PointCloud::ConstPtr& cloud_in)
{
	...
	else
	{
		...
		if(updateMeas)
		{
			...
			ROS_DEBUG("distTolastCurb: %5f, curb accumNum: %d, left crossing: %ld, right crossing: %ld", distTolastCurb, curbdata_->accumNum_, LeftCroData_->FakeSensorPose_.size(), RightCroData_->FakeSensorPose_.size());
            ...
            if(CurbUseFlag_||CrossingUseFlag_)
            {
                ...
                if(!(++resample_count_ % resample_interval_))
                {
                    ...
                    //20120110 disable random particle injection;
                    pf_->w_diff_setvalue=0.00;
...
void MultiAmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
	...
	pf_vector_t laser_delta = pf_vector_zero();
	pf_vector_t odom_delta = pf_vector_zero();
	...
	if(laser_init_)
	{
		...
		laser_delta.v[0] = pose.v[0] - pf_laser_odom_pose_.v[0];
		laser_delta.v[1] = pose.v[1] - pf_laser_odom_pose_.v[1];
		laser_delta.v[2] = angle_diff(pose.v[2], pf_laser_odom_pose_.v[2]);
		
		odom_delta.v[0] = pose.v[0] - pf_odom_pose_.v[0];
		odom_delta.v[1] = pose.v[1] - pf_odom_pose_.v[1];
		odom_delta.v[2] = angle_diff(pose.v[2], pf_odom_pose_.v[2]);
		
		distTolastLaser = sqrt(laser_delta.v[0]*laser_delta.v[0]+laser_delta.v[1]*laser_delta.v[1]);
		distTolastOdom = sqrt(odom_delta.v[0]*odom_delta.v[0]+odom_delta.v[1]*odom_delta.v[1]);
	...
	{
		ROS_DEBUG("---------------------update Meas----------------");
		ROS_DEBUG("distTolastLaser: %5f", distTolastLaser);
        ...
        */ 
		...
		if(LaserUseFlag_)
		{
			...
			ROS_DEBUG("cloud_msg size: %ld", cloud_msg.poses.size());
			particlecloud_pub_.publish(cloud_msg);
		}
		else
		{
...
void MultiAmclNode::markerReceived (const vision_lane_detection::markers_info::ConstPtr& markers_in)
{
	ROS_INFO("---------------marker information received---------------");
	ros::Time markerRece_time = markers_in->header.stamp;
	ros::Time now_marker = ros::Time::now();
	ROS_INFO("marker: time now %lf, marker %lf", now_marker.toSec(), markerRece_time.toSec());
    ...
    try
    {
        tf_->lookupTransform(global_frame_id_, base_frame_id_, markerRece_time, baseMapTemp);
        prior_tf_available       = true;
        validate_function_switch = true;
    }
    catch(tf::TransformException e)
    {
        ROS_WARN_THROTTLE(1, "Markers Failed to get fresh tf between map and baselink, (%s)", e.what());
        //return;
    }
    
    if(markers_in->vec.size()==0)
    {
		ROS_INFO("---------------empty info---------------");
        MarkerUseFlag_ = false;
        if(latest_tf_valid_)
        {
            ros::Time transform_expiration = (markers_in->header.stamp + transform_tolerance_);
            tf::StampedTransform tmp_tf_stamped(latest_tf_.inverse(), transform_expiration, global_frame_id_, odom_frame_id_);
            this->tfb_->sendTransform(tmp_tf_stamped);
        }
        return;
	...
	ROS_INFO("---------------useful markers detected---------------");
	
	// Where was the robot when this marker was taken?
	tf::Stamped<tf::Pose> odom_pose;
	pf_vector_t pose;
	double pitch;
	if(!getOdomPose(odom_pose, pose.v[0], pose.v[1], pose.v[2], pitch, markers_in->header.stamp, base_frame_id_))
	{
		ROS_ERROR("Couldn't determine robot's pose associated with marker input");
		return;
	}
	pf_mutex_.lock();

	pf_vector_t marker_delta = pf_vector_zero();
	pf_vector_t odom_delta = pf_vector_zero();
	double distTolastOdom=0.0;
	double distTolastMarker=0.0;
	...
	if(marker_init_)
	{
		// Compute change in pose
		//delta = pf_vector_coord_sub(pose, pf_odom_pose_);
		marker_delta.v[0] = pose.v[0] - pf_marker_odom_pose_.v[0];
		marker_delta.v[1] = pose.v[1] - pf_marker_odom_pose_.v[1];
		marker_delta.v[2] = angle_diff(pose.v[2], pf_marker_odom_pose_.v[2]);
		
		odom_delta.v[0] = pose.v[0] - pf_odom_pose_.v[0];
		odom_delta.v[1] = pose.v[1] - pf_odom_pose_.v[1];
		odom_delta.v[2] = angle_diff(pose.v[2], pf_odom_pose_.v[2]);
		
		distTolastMarker = sqrt(marker_delta.v[0]*marker_delta.v[0]+marker_delta.v[1]*marker_delta.v[1]);
		distTolastOdom = sqrt(odom_delta.v[0]*odom_delta.v[0]+odom_delta.v[1]*odom_delta.v[1]);
		
		// See if we should update the filter
		update   =	fabs(marker_delta.v[0]) > marker_d_thresh_ ||
					fabs(marker_delta.v[1]) > marker_d_thresh_ ||
					fabs(marker_delta.v[2]) > marker_a_thresh_;
	...
	else if(marker_init_ && update)
	{
		ROS_INFO("---------------------marker informaiton update----------------");
		ROS_DEBUG("distTolastOdom: %5f", distTolastOdom);
        ...
        if(prior_tf_available)
        {
            ...
            //printf("pf estimation pose:(%3f, %3f, %3f)\n", pf_->Pos_Est.v[0], pf_->Pos_Est.v[1], pf_->Pos_Est.v[2]);
        }
	}
	...
	if(update)
	{
		visionMarkerData_->sensor = visionMarker_;
		visionMarkerData_->Pos_Est = pf_->Pos_Est;
		bool UseFlagTmp;
		bool *FlagPointer = &UseFlagTmp;
		MarkerUseFlag_ = false;
		
        sensor_msgs::PointCloud temp;
        void *src = &temp;
        
        //use message to update;
        for(size_t m=0; m < markers_in->vec.size(); m++)
        {
			visionMarkerData_-> marker_meas_.marker_type = markers_in->vec[m].class_label;
			visionMarkerData_-> marker_meas_.maker_pose.v[0] = markers_in->vec[m].x;
			visionMarkerData_-> marker_meas_.maker_pose.v[1] = markers_in->vec[m].y;
			visionMarkerData_-> marker_meas_.maker_pose.v[2] = markers_in->vec[m].thetha;
			visionMarker_->UpdateSensor(pf_, (AMCLSensorData*)visionMarkerData_, FlagPointer, validate_function_switch, src);
			if(UseFlagTmp) MarkerUseFlag_= true;
		}
		...
		if(MarkerUseFlag_)
		{
			ROS_DEBUG("------------Marker Marker Marker----------");
			resample_interval_= 2;
			// Resample the particles
			if(!(++resample_count_ % resample_interval_))
			{
			...
			{
				ROS_DEBUG("Marker_resample_count: %d", resample_count_);
			}
	...
	if(resampled || force_publication)
	{
		...
		if(max_weight > 0.0)
		{
			...
			// Fill in the header
			p.header.frame_id = global_frame_id_;
			p.header.stamp = markers_in->header.stamp;
			// Copy in the pose
			p.pose.pose.position.x = hyps[max_weight_hyp].pf_pose_mean.v[0];
			p.pose.pose.position.y = hyps[max_weight_hyp].pf_pose_mean.v[1];
			tf::quaternionTFToMsg(tf::createQuaternionFromYaw(hyps[max_weight_hyp].pf_pose_mean.v[2]),
...
double MultiAmclNode::getYaw(tf::Pose& t)
{
	double yaw, pitch, roll;
...
void MultiAmclNode::initialPoseReceivedOld(const geometry_msgs::PoseWithCovarianceStampedConstPtr& msg)
{
...
void MultiAmclNode::initialPoseReceived(const geometry_msgs::PoseWithCovarianceStampedConstPtr& msg)
{
