...
private:
	...
	bool Curb_Noise_Filtering( tf::StampedTransform &odom_meas_curb_old,
			std::vector <geometry_msgs::Point32> & curb_line,
			geometry_msgs::Point32 &meas_pt,
	...
	// include the custom message output to share useful road surface data
	ros::Publisher								curb_pt_details_pub_;
	ros::Publisher								laser_cloud_laser_pub_;
  ...
  diag_odo_("odo input", diagnostic_updater_, diag_freq_odo_param_)
{
	...
	curb_pt_details_pub_=	nh_.advertise<robust_road_detection::curb_details>("curb_details",2);
	laser_cloud_laser_pub_=	nh_.advertise<sensor_msgs::PointCloud>("laser_cloud_laser", 2);
...
void curb_detect::scanCallback (const sensor_msgs::LaserScan::ConstPtr& scan_in)
{
	...
	laser_cloud_laser_pub_.publish(laser_cloud_laser_);
...
void curb_detect::Curb_Verification()
{
	...
	robust_road_detection::curb_details curb_pt_details;
	curb_pt_details.header = laser_cloud_base_.header;
	curb_pt_details.left_pt_id = -1;
	curb_pt_details.right_pt_id = -1;
	if(right_curb_)
	{
		...
		if(RCurb_Noise_Filtering(rightCurb_odom_pt, serial_number)==false)
		{
			...
			curb_pt_details.right_curb = laser_cloud_base_.points[right_curb_serial];
			curb_pt_details.right_pt_id = right_curb_serial;
		}
	}
	if(left_curb_)
	{
		...
		if(LCurb_Noise_Filtering(leftCurb_odom_pt, serial_number)==false)
		{
			...
			curb_pt_details.left_curb = laser_cloud_base_.points[left_curb_serial];
			curb_pt_details.left_pt_id = left_curb_serial;
		}
	}
	...
	curb_pt_details_pub_.publish(curb_pt_details);
    cout<<"Finished publish"<<endl;
}
...
void curb_detect::publish_control()
{
	...
	if(pub_init_)
	{
        ...
        cout<<"mov_dis: "<<mov_dis<<" tx: "<<tx<<" ty: "<<ty<<endl;
	}
	else
	{
		pub_init_ = true;
    ...
    cout<<"Publish flag "<<publish_flag_<<endl;
