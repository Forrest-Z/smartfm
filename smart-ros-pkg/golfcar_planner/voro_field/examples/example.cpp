...
int main( int argc, char *argv[] ) {
 ...
 // voronoi.visualize("initial.ppm", "voro_initial.ppm");
 // std::cerr << "Generated initial frame.\n";
  ...
  for (int frame=1; frame<=5; frame++) {
    ...
    */
    voronoi.exchangeObstacles(empty); // register the new obstacles (old ones will be removed)
    voronoi.update();
  ...
  voronoi.update();
  if (doPrune) voronoi.prune();
  voronoi.visualize("obstacle_dist.ppm", "voro_dist.ppm", "voro_field.ppm");
