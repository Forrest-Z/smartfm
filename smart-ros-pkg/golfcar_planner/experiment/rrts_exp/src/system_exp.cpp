...
double SystemExp::extend_dubins_spheres (double x_s1, double y_s1, double t_s1,
		double x_s2, double y_s2, double t_s2, int combination_no,
  ...
  // different costs
	...
	double time_cost = 0.5*((t_increment_s1 + t_increment_s2) * turning_radius  + line_distance);
...
int SystemExp::extendTo (StateExp &stateFromIn, StateExp &stateTowardsIn,
	...
	if((min_cost <= 0.0) || (min_cost > DBL_MAX/2)){
		delete[] end_state;
		return 0;
	}
