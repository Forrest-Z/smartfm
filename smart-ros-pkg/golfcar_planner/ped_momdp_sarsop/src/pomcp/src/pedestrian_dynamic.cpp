...
bool PEDESTRIAN_DYNAMIC::Step(STATE& state,int action, OBS_TYPE & observation,double&reward) const
{
	PEDESTRIAN_DYNAMIC_REAL_STATE& pedestrian_state=safe_cast<PEDESTRIAN_DYNAMIC_REAL_STATE&>(state);
	reward=0;
	observation=1000;
...
PEDESTRIAN_DYNAMIC_REAL::PEDESTRIAN_DYNAMIC_REAL(bool qmdp,int size=9):Size(size)
{
	//Discount=1.0;
	Discount=0.95;
	//NumObservations=5*Size+1;
...
bool PEDESTRIAN_DYNAMIC_REAL::Step(STATE& state,int action, OBS_TYPE& observation,double&reward) const
{
	PEDESTRIAN_DYNAMIC_REAL_STATE& pedestrian_state=safe_cast<PEDESTRIAN_DYNAMIC_REAL_STATE&>(state);
	reward=0;
	observation=1000;
	...
	reward=-50;
	//reward=0;
	if(UseQmdp==false)  //partially observable
	{
		observation=rob_vel*500+robY*41+pedX*11+pedY;
	}
	else   //fully observable
	{
		observation=goal*1600+rob_vel*500+robY*41+pedX*11+pedY;
	}
