...
bool TIGER::StepQMDP(STATE& state,int action, OBS_TYPE & observation,double&reward) const
{
	TIGER_STATE& tigerstate=safe_cast<TIGER_STATE&>(state);
	...
	else if(action==OPEN_LEFT)
	{
		...
		if(Bernoulli(0.5)) 
		{
			tigerstate.tigerpos=0;
			observation=0;
		}
		else 	
		{
			tigerstate.tigerpos=1;
			observation=1;
		}
		return false;
	}
...
bool TIGER::Step(STATE& state,int action, OBS_TYPE & observation,double&reward) const
{
	if(UseQmdp==true)  return StepQMDP(state,action,observation,reward);
	TIGER_STATE& tigerstate=safe_cast<TIGER_STATE&>(state);
	reward=0;
	double noise=0.15;
	if(action==LISTEN)
