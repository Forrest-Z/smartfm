    ...
    int& observation, double& reward) const
{
    ...
    // _Observation occurs in final positions, not start positions
    ...
    int stepObs, const STATUS& status) const
{
    ...
    int simObs = GetObservation(tagstate, history.Back()._Action);
    ...
    vector<int>& actions, const STATUS& status) const
{
    ...
    if (history.Back()._Observation == NumCells && IsCorner(tagstate.AgentPos))
    {
        actions.push_back(4);
        return;
    }
    ...
    for (int d = 0; d < 4; ++d)
        if (history.Back()._Action != COORD::Opposite(d)
