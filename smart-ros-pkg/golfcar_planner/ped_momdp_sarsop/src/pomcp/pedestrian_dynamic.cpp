...
{
	PEDESTRIAN_DYNAMIC_STATE& pedestrian_state=safe_cast<PEDESTRIAN_DYNAMIC_STATE&>(state);
	reward=0;
	observation=1000;
...
STATE*PEDESTRIAN_DYNAMIC_REAL::CreateStartState() const
{
	...
	int rand_pose=rand()%40;
	PEDESTRIAN_DYNAMIC_REAL_state->PedPos.X=rand_pose/10;
	PEDESTRIAN_DYNAMIC_REAL_state->PedPos.Y=rand_pose-PEDESTRIAN_DYNAMIC_REAL_state->PedPos.X*10;
...
void PEDESTRIAN_DYNAMIC_REAL::InitModel()
{
	for(int i=0;i<4;i++)
	{
		if(i==1) continue;
