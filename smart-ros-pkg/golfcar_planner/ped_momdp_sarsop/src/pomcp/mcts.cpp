...
//	pedproblem_d->vnode_list.push_back(Root);
//	pedproblem_d->history_list.push_back(History);
...
{		
	...
	cout<<"history list size "<<pedproblem_d->history_list.size()<<endl;
	cout<<"vnode list size "<<pedproblem_d->vnode_list.size()<<endl;
	//cout<<"vnode old size "<<pedproblem_d->vnode_old.size()<<endl;
	reuse_count=0;
	merge_count=0;
	root_count=0;
	o=l=r=sum=0;
	...
	for(int i=0;i<pedproblem_d->history_list.size();i++)
	{
		HISTORY h=pedproblem_d->history_list[i];
		for(int i=0;i<h.Size();i++)
			merge_out<<"a:"<<h[i]._Action<<" o:"<<h[i]._Observation<<" ";	
		merge_out<<endl;
		VNODE*vn=pedproblem_d->vnode_list[i];
		pedproblem_d->DisplayBeliefs(vn->Beliefs(),merge_out);
	}*/
	//std::copy(pedproblem_d->vnode_list.begin(),pedproblem_d->vnode_list.end(),pedproblem_d->vnode_old.begin());
//	pedproblem_d->vnode_old.clear();
//	pedproblem_d->vnode_old.insert(pedproblem_d->vnode_old.begin(),pedproblem_d->vnode_list.begin(),pedproblem_d->vnode_list.end());
	pedproblem_d->vnode_list.clear();
	pedproblem_d->history_list.clear();
...
//	pedproblem_d->UpdateModel(0);
    ...
    Root = newRoot;
	Rollout_Root=vnode;
	pedproblem_d->DisplayBeliefs(beliefs,cout);
	...
	int px=PEDESTRIAN_DYNAMIC_state->PedPos.X;
	int py=PEDESTRIAN_DYNAMIC_state->PedPos.Y;
	cout<<"px py "<<px<<" "<<py<<endl;
	pedproblem_d->map[px][py]=0;
	cout<<endl;
	for(int i=0;i<4;i++)
	{
		for(int j=0;j<10;j++)
			cout<<pedproblem_d->map[i][j];
		cout<<endl;
	}
...
void MCTS::UCTSearch()
{
	...
	cout<<"rollout_time"<<rollout_time<<endl;
	rollout_time=0;
	merge_time=0;
	map_time=0;
    ClearStatistics();
    ...
    for (n = 0; n < Params.NumSimulations; n++)
    {
		...
		curr_sim=n;
		//cout<<"start creating sample"<<endl;
        STATE* state = Root->Beliefs().CreateSample(Simulator);
	...
	//cout<<"map time "<<pedproblem_d->map_time<<endl;
	//cout<<"map count"<<pedproblem_d->map_counter<<endl;
	...
	for(int i=0;i<pedproblem_d->history_list.size();i++)
	{
		...
		merge_out<<endl;
		pedproblem_d->DisplayBeliefs(pedproblem_d->vnode_list[i]->Beliefs(),merge_out);
	}*/
...
VNODE* MCTS::VMerge(VNODE *n1, VNODE*n2)
{
	...
	{	
			...
			for(int j=0;j<n2->Children[i].Children.size();j++)
			{
...
void MCTS::Merge(VNODE *n1, VNODE *n2)
{
	...
	/*
	n1->merged=true;
	n1->merge_index=pedproblem_d->FindIndex(n2);
	*/
...
double MCTS::SimulateQ(STATE& state, QNODE& qnode, int action)
{
	...
	if (!terminal)
	{
		...
		//	L2=tv.tv_sec*1000*1000+tv.tv_usec;
		//	map_time+=(L2-L1);
		//VNODE* vnode_new=pedproblem_d->MapBelief(vnode,Root,History);
		//cout<<"belief mapped"<<endl;
	//	bool qmdp = true;
		if(Params.UseQmdp)
		{
			//cout<<"Qmdp enabled"<<endl;
			//reserved	if(qmdp) 			
			//delayedReward=pedproblem_d->QMDP(&state);
			delayedReward=Rollout(state);
		}
		else if(!vnode)
		{
		...
		{
			//vnode_new=pedproblem_d->MapBelief(vnode,Root,History);
			//cout<<"start map belief"<<endl;
			//boost::timer;		

			if(enable_reuse&&(vnode->merged==true))
			{
				reuse_count++;
			...
			//vnode_new=pedproblem_d->MapBelief(vnode,Root,History);
			else if(enable_reuse&&(vnode_new=pedproblem_d->MapBelief(vnode,Root,History,historyDepth))&&vnode!=vnode_new)
			{
				merge_count++;	
				//SimulateV(state,vnode);
				/*
				...
				merge_out<<endl;
				HISTORY h=pedproblem_d->history_list[pedproblem_d->FindIndex(vnode_new)];
				end=h.Size();
				for(int i=begin;i<end;i++)
				{
					merge_out<<"a:"<<h[i]._Action<<" o:"<<h[i]._Observation<<" ";	
				...
				pedproblem_d->DisplayBeliefs(vnode->Beliefs(),merge_out);
				merge_out<<"vnode_new beliefs :"<<endl;
				pedproblem_d->DisplayBeliefs(vnode_new->Beliefs(),merge_out);

				
				//cout<<"merge finished"<<endl;
				//SimulateV(state,vnode_new);
				delayedReward=(vnode_new)->Value.GetValue();	


			}
			else
				//vnode_new=pedproblem_d->FindByIndex(vnode->merge_index);
			{		
