...
map<string, string> Client::processTurnMes(string mes)
{
	...
	TiXmlElement * pNextSibling = root->FirstChild("observed-fluent")->ToElement();
	while(pNextSibling != NULL)
	{
		TiXmlElement * pName = pNextSibling->FirstChild("fluent-name")->ToElement();
		TiXmlElement * pValue = pNextSibling->FirstChild("fluent-value")->ToElement();
...
string Client::createActionMes(const string actionName, const string actionValue1)
{
	TiXmlDocument doc;
