...
RandomStreams::RandomStreams(vector<unsigned> seeds, int length)
	: position_(0) {
	int num_streams = seeds.size();
...
int RandomStreams::NumStreams() const { return streams_.size(); }

int RandomStreams::Length() const { return streams_.size() > 0 ? streams_[0].size() : 0; }

void RandomStreams::Advance() const { position_++; }
void RandomStreams::Back() const { position_--; }
