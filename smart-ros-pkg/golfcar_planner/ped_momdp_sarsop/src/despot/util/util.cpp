...
mutedostream::mutedostream() :
	std::ostream(&buffer), buffer(std::cout) {
}

mutedostream::StreamBuffer::StreamBuffer(std::ostream& str) :
	out(str) {
}

std::ostream& operator<<(std::ostream &out, void* arg) {
	return out;
}

int mutedostream::StreamBuffer::sync() {
	return 0;
}
