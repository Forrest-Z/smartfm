...
Floor::Floor() : num_rows_(0), num_cols_(0) {}
...
Coord Floor::GetCell(int i) const { return cells_[i]; }

bool Floor::Inside(Coord coord) const {
	return Inside(coord.x, coord.y);
}
bool Floor::Inside(int x, int y) const {
	return (x >= 0 && x < num_cols_ && y >=0 && y < num_rows_) &&  floor_[y][x] != INVALID;
}
...
vector<double> Floor::ComputeDistances(int source) {
	...
	while (!list.empty()) {
		...
		for (int dir=0; dir<4; dir++) {
			...
			if (GetIndex(next) != INVALID && distMatrix[next.y][next.x] > distMatrix[cur.y][cur.x] + 1) {
...
int Floor::num_rows() const { return num_rows_; }
int Floor::num_cols() const { return num_cols_; }
int Floor::NumCells() const { return cells_.size(); }
