...
/*---------------------------------------------------------------------------*/
const Coord Compass::DIRECTIONS[] = {
	Coord(0, 1),
	Coord(1, 0),
	Coord(0, -1),
	Coord(-1, 0),
...
const string Compass::CompassString[] = {
	"N",
	"E",
	"S",
	"W",
...
int Coord::DirectionalDistance(Coord lhs, Coord rhs, int direction) {
	switch (direction) {
		case Compass::NORTH: return rhs.y - lhs.y;
		case Compass::EAST: return rhs.x - lhs.x;
		case Compass::SOUTH: return lhs.y - rhs.y;
		case Compass::WEST: return lhs.x - rhs.x;
		default: assert(false);
	}
}

int Compass::Opposite(int dir) { return (dir + 2) % 4; }
