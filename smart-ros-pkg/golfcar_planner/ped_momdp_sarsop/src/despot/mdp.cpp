...
void MDP::ComputeOptimalPolicyUsingVI() {
  ...
  clock_t start = clock();
	...
	while (true) {
		for (int s=0; s<num_states; s++) {
			...
			for (int a=0; a<num_actions; a++) {
				...
				for (const auto& next : transition) {
					assert(next.state_id >= 0);
					v += next.weight * Discount() * policy_[next.state_id].value;
				}
	...
	cerr <<"Done [" << iter << " iters, tol = " << diff << ", " << (double)(clock() - start) / CLOCKS_PER_SEC << "s]!" << endl;
}
...
void MDP::ComputeBlindAlpha() {
	...
	for (int action=0; action<num_actions; action++) {
		...
		for (iter=0; iter<1000; iter++) {
			...
			for (int s=0; s<num_states; s++) {
				cur[s] = 0;
				const vector<State>& transition = TransitionProbability(s, action);
				for (const auto& next : transition) {
					assert(next.state_id >= 0);
		...
		cerr << "Tol(alpha_" << action << ") after " << iter << " iters = " << tol << endl;

		blind_alpha_[action] = cur;
	}
}

double MDP::ComputeActionValue(const ParticleBelief* belief, const StateIndexer& indexer, int action) const{
	assert(blind_alpha_.size() != 0);

	const vector<State*> particles = belief->particles();
	double value = 0;
	for (State* particle : particles) {
		value += particle->weight * blind_alpha_[action][indexer.GetIndex(particle)];
	}
