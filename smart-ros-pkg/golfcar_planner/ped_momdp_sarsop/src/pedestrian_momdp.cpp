...
pedestrian_momdp::pedestrian_momdp(int argc, char** argv) {
	ros::NodeHandle nh;
	robotSub_ = nh.subscribe("robot_0/base_pose_ground_truth", 1, &pedestrian_momdp::robotPoseCallback, this);
	pedSub_ = nh.subscribe("robot_1/base_pose_ground_truth", 1, &pedestrian_momdp::pedPoseCallback, this);
	//subscribe as heartbeat of the robot
	scanSub_ = nh.subscribe("robot_0/base_scan", 1, &pedestrian_momdp::scanCallback, this);
...
void pedestrian_momdp::robotPoseCallback(nav_msgs::Odometry odo)
{
...
void pedestrian_momdp::pedPoseCallback(nav_msgs::Odometry odo)
{
	pedx_ = odo.pose.pose.position.x;
	pedy_ = odo.pose.pose.position.y;
	cout<<pedx_<<' '<<pedy_<<endl;
	obs_flag = true;
}

void pedestrian_momdp::scanCallback(sensor_msgs::LaserScan laser)
{
	...
	if(moveRob)
	{
		...
		if(cmd.linear.x>=3) cmd.linear.x = 3;
		cmdPub_.publish(cmd);
	}
...
{
	ros::init(argc, argv, "ped_momdp_sarsop");
