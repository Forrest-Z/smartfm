...
local_frame::local_frame()
{
    ...
    n.param("threshold", threshold_, 0.0);
    n.param("offsetx", offsetx_, 0.0);
    n.param("offsety", offsety_, 0.0);
    ...
    for(size_t i=0;i<ped_batch->pd_vector.size();i++)
    {
        ...
        if(matched)
        {
            ...
            //if(ped_batch->pd_vector[i].confidence > threshold_/100)
            {
                ...
                plf.ped_pose.x = ped_point.x;//out_pose.getOrigin().getX();
                plf.ped_pose.y = ped_point.y;//out_pose.getOrigin().getY();
                plf.ped_pose.z = ped_point.z;//out_pose.getOrigin().getZ();
