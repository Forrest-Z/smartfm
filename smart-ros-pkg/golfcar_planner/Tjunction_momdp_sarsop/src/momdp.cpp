...
void ped_momdp::controlLoopConstantGap2(const ros::TimerEvent &e)
{
    ...
    else//Wait for a certain amount of gap in order to merge!
    {
		...
		safeAction=1;
			for(int ii=0; ii<lPedInView.size(); ii++)
			{
			if( lPedInView[ii].ped_pose.x < 5 &&  lPedInView[ii].ped_pose.x > -8 )
				{
					   safeAction = 0;
					   volkan.linear.x = lPedInView[ii].ped_pose.x;
					   volkan.linear.y = roboty_;
...
void ped_momdp::controlLoopConstantGap3(const ros::TimerEvent &e)
{
	int safeAction=1;
	    ...
	    else if(getYGrid(roboty_)>3 )//After merging the way, don't stop!
	    {
	    	safeAction=1;
	    ...
	    else//Wait for a certain amount of gap in order to merge!
	    {
			...
			safeAction=1;
				for(int ii=0; ii<lPedInView.size(); ii++)
				{
				if( lPedInView[ii].ped_pose.x < 5 &&  lPedInView[ii].ped_pose.x > -35 )
					{
						   safeAction = 0;
						   volkan.linear.x = lPedInView[ii].ped_pose.x;
						   volkan.linear.y = roboty_;
