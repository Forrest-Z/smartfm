...
void ped_momdp::controlLoopConstantGap1(const ros::TimerEvent &e)
{
    ...
    cout << "Control loop dj .. " << " lPedInView " << lPedInView.size() << endl;
    if(robotz_<7 )//Approach to the intersection area without any other concern!
    {
    	safeAction=1;
    }
    else if(robotz_>=12 )//After merging the way, don't stop!Only condition you can stop is there is  a car in front of you is very near to you
    {		
    	safeAction=1;
    		for(int ii=0; ii<lPedInView.size(); ii++)
			{
			if( (lPedInView[ii].ped_pose.z-robotz_) > 30 && (lPedInView[ii].ped_pose.z-robotz_) < 35 )
				{
					   safeAction = 0;
				}
			}
    ...
    else//Wait for a certain amount of gap in order to merge!
    {
		...
		safeAction=1;
			for(int ii=0; ii<lPedInView.size(); ii++)
			{
			if( lPedInView[ii].ped_pose.z > 15 &&  lPedInView[ii].ped_pose.z < 45 )
				{
					   safeAction = 0;
				}
			}
