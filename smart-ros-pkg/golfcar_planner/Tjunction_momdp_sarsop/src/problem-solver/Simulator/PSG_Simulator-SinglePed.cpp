...
/// Initialize online execution
    ...
    for(;;)
    {
		...
		if(mostLikelyAlgo)
		{
			...
			//belief_vector currMLBel;
			


			//for(int ii=0; ii <   currMLBel->bvec->size(); ii++)
			{
				//if(currMLBel->bvec->data[ii].index == mostProbY)
					//currMLBel->bvec->data[ii].value = 1.0;
				//else
					//currMLBel->bvec->data[ii].value = 0.0;
