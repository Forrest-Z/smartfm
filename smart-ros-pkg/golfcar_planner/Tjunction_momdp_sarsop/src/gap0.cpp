...
ped_momdp::ped_momdp(string model_file, string policy_file, int simLen, int simNum, bool stationary, double frequency, bool use_sim_time, ros::NodeHandle& nh)
{
    ...
    timer_ = nh.createTimer(ros::Duration(1.0/frequency), &ped_momdp::controlLoopConstantGap0, this);
...
/////////////////////CONTROL LOOP FOR CONSTANT GAP-0///////////////////////////////////////////////////////
void ped_momdp::controlLoopConstantGap0(const ros::TimerEvent &e)
{
	int safeAction=1;
    ...
    else//Wait for a certain amount of gap in order to merge!
    {
		...
		safeAction=1;
			for(int ii=0; ii<lPedInView.size(); ii++)
			{
			if( lPedInView[ii].ped_pose.x < 5 &&  lPedInView[ii].ped_pose.x > -20 )
				{
					   safeAction = 0;
