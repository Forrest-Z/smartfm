...
private:
    int X_SIZE, Y_SIZE;
    double dX, dY, dgrid;
    int RobInitSt,DrivInitSt;
    double momdp_problem_timeout;
    bool robot_pose_available;
    double robotx_, roboty_, robotz_, robotspeedx_, momdp_speed_;
    ...
    int getCurrentState(double drob, double ddriv, int rob_init_st, int driv_init_st);

    int getCurrObs(int id,int driv_init_st);

    int getXGrid(double x);

    int getYGrid(double y);
    
    int getStGrid(double dist);
};
#endif /* MOMDP_H_ */
