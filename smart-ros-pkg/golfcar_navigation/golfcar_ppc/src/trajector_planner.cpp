...
bool PurePursuitBase::computeVelocityCommands(geometry_msgs::Twist& cmd_vel){
    ...
    if(path_flag_){
        ...
        vector<geometry_msgs::Point32> critical_polygon;
        ...
        for(unsigned int i=0;i<observed_polygon_cells.size();i++)
        {
            ...
            //cout<<observed_polygon_cells[i].x<<"\t"<<observed_polygon_cells[i].y<<"\t"<<cells_cost<<"\n";
        ...
        for(unsigned int i=0;i<critical_polygon_cells.size();i++)
        {
            ...
            if(cells_cost==254)
            {
                ...
                cout<<"Hazard, obstacle in front! Stopping...."<<obstacle_x<<" "<<obstacle_y<<"\n";
                cmd_vel.linear.x = 0;
                obstacle++;
            }
            else
