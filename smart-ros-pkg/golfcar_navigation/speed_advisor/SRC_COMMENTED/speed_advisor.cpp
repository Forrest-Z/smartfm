...
SpeedAdvisor::SpeedAdvisor() {
    ...
    // Sets up speed-related members
    
    nh.param( "max_speed", high_speed_, 2.0);
    
    nh.param( "min_speed", min_speed_, 0.15); // speed_controller doesn't use acceleration pedal when below 0.1, so put min_speed_ > 0.1
    ...
    // Sets up acceleration-related members
    ...
    timer_ = nh_.createTimer( ros::Duration( 1.0 / frequency_ ), &SpeedAdvisor::ControlLoop, this);
    
    // Initializes named loggers so that they the log level can be set in ros-console
