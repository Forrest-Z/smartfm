...
namespace sensing_on_road{
   ...
   float pt_distance(const geometry_msgs::Point32 & p1, const geometry_msgs::Point32 & p2)
   {
    ...
    void laser_extraction::connect_segments()
    {
        ...
        for(unsigned is=0; is< extracted_segments_laser_.segments.size()-1; is++)
        {
            for(unsigned ic = is+1; ic< extracted_segments_laser_.segments.size(); ic++)
            {
                unsigned last_serial_temp = extracted_segments_laser_.segments[is].beam_serials.back();
                float range_temp = laser_scan_.ranges[last_serial_temp];
                float dis_threshold = 0;
                if(range_temp<DIS_RANGE){dis_threshold = ASSOCIATE_WITHIN_RANGE_THRESH;}
                else{dis_threshold = ASSOCIATE_OVER_RANGE_MULTI * range_temp * laser_scan_.angle_increment;}
                
                unsigned serial_number1 = extracted_segments_laser_.segments[is].point_serials.back();
                unsigned serial_number2 = extracted_segments_laser_.segments[ic].point_serials.front();
    ...
    void laser_extraction::delete_segments()
    {
        ...
        for(unsigned dc=0; dc<delete_connected_segments_.size(); dc++)
        {
            ...
            if(delete_connected_segments_[dc]<0){ROS_INFO("Unexpected Scenario");}
            unsigned delete_seg_serial = (unsigned)delete_connected_segments_[dc];
            ...
            // Or we can use a plain class rather than a message, it seems that
