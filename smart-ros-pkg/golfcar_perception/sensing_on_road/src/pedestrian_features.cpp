...
namespace sensing_on_road{
    ...
    void pedestrian_features::pedestrian_filtering(const sensing_on_road::segments_one_batch& batch_para)
    {
        ...
        /*
        for(unsigned int is =0; is <filtered_segment_batch_.segments.size(); is++)
        {
        ...
        else
        {
            ...
            for(unsigned int ih=0; ih!=history_pool_.size(); ih++) 
            {
                ...
                history_pool_[ih].nearest_distance = disSpace-compensate_dis;
                 ...
                 ROS_INFO("1--history obj_label %d, serial %d, %3f, %3f; segment %d, %3f, %3f, shortest dis2d_tmp, distance: %3f, %3f", 
            ...
            std::vector<unsigned int> merged_history_label;
            ...
            for(unsigned int ih=0; ih!=history_pool_.size(); ih++) 
            {
                ...
                else if(history_pool_[ih].history_status==2)
                {
                    ...
                    for(unsigned int io=0; io < history_pool_[ih].merged_labels.size(); io++)
                    {
                    ...
                    for(unsigned int y = 0; y < history_pool_[history_serial].merged_labels.size(); y++)
                    {
                        for(unsigned int k = 0; k < history_pool_[history_serial].merged_labels.size()-1-y; k++)
                        {
            ...
            for(unsigned int is=0; is!=filtered_segment_batch_.segments.size(); is++)
            {
                ...
                if(filtered_segment_batch_.segments[is].segment_status==3)
                {
                    ...
                    for(unsigned int il=0; il<history_tmp.merged_labels.size(); il++)
                    {
            ...
            for(unsigned int im=0; im!=merged_history_label.size(); im++) 
            {
                for(unsigned int ih=0; ih!=history_pool_.size(); ih++) 
                {
    ...
    {
        ros::Time segment_time = filtered_segment_batch_.header.stamp;
        std::vector<unsigned int> stale_history;
        for(unsigned int ih=0; ih!=history_pool_.size(); ih++) 
        {
            //1st: maintain size;
            ...
            {stale_history.push_back(ih);ROS_INFO("Delete History %d", ih);continue;}
        ...
        {
            for(unsigned int is = stale_history.size(); is!=0; is--)
            {               
                unsigned int delete_history_serial = stale_history[is-1];
            ...
            else
            {
                ...
                if(history_pool_[ih].fastest_velocity<3.0 && history_pool_[ih].single_segment_history.back().max_dimension<3.0 )
                {
                    ...
                    if(ped_pub)
                    {
                        ...
                        ROS_INFO("-------------pedestrian history object label %d, serial %d--------------", history_pool_[ih].object_label, ih);
                    }
                }
                else
                {
   ...
   void pedestrian_features::Bayesfilter(const sensing_on_road::pedestrian_vision_batch& veri_pd_para)
    {
        ...
        for(unsigned int ih=0; ih!=history_pool_.size(); ih++) 
        {
            for(unsigned int i=0; i<veri_pd_para.pd_vector.size(); i++)
            {
    ...
    void pedestrian_features::ProbabilityCheck()
    {
        ...
        for(unsigned int ih=0; ih!=history_pool_.size(); ih++) 
        {
            ...
            if(history_pool_[ih].pedestrian_belief > PROBABILITY_THRESHOLD)
            {
                ...
                for(unsigned int ib=0; ib<veri_batch_.pd_vector.size(); ib++)
                {
