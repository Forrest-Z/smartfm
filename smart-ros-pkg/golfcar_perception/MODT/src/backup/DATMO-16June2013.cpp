...
private:
	...
	ros::Publisher                              			vehicle_array_pub2_;
	//vehicle local image;
...
: private_nh_("~")
{
	...
	vehicle_array_pub2_		=   nh_.advertise<geometry_msgs::PoseArray>("vehicle_array2", 2);
	...
	for(size_t i=0;  i<appear_disappear_pairs.size(); i++)
	{
		...
		if(appear_serial>=0 && disappear_serial>=0)
		{
			...
			if(area_appear_tmp > 10.0 && area_disappear_tmp > 10.0)
			{
				...
				if(current_rect.size()==0) {ROS_INFO("current_rect.size() = 0");continue;}
				else
				{
					ROS_INFO("Process current rect");
					...
					*/
					...
					if(vehicle_flag)
					{
						...
						pair<pair<Point2f, Point2f>, float> pose_pair_tmp;

						pose_pair_tmp = make_pair(make_pair(candidate_conner_points[0], candidate_conner_points[1]), moving_direction);
						vehicle_poses_img.push_back(pose_pair_tmp);
...
void DATMO::vehicle_pose_generation(vector<pair<pair<Point2f, Point2f>, float> > &vehicle_poses_img)
{
	...
	vehicle_array_pub2_.publish(vehicle_array);
}
