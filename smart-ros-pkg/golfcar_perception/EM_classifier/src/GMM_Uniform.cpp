...
int main( int /*argc*/, char** /*argv*/ )
{
    ...
    //double uniform_fi;
    double uniform_area = 200.0*200.0;
    Mat uniAreaCenter(1, 2, CV_64FC1);
    uniAreaCenter.at<double>(0, 0) = 300.0;
    uniAreaCenter.at<double>(0, 1) = 300.0;
    ...
    for(iteration_time = 0; iteration_time < max_iteration; iteration_time++)
    {
    	...
    	double nearest_distance = 100000.0;
    	int cluster_label = 0;
    	for(i=0; i<N+1; i++)
		{
    		printf("means (%lf, %lf);\t fi:%lf\n", gaussian_means[i].at<double>(0,0), gaussian_means[i].at<double>(0,1), gaussian_fi[i]);
    		if(norm(gaussian_means[i], uniAreaCenter)<nearest_distance)
    		{
    			cluster_label=i;
    			nearest_distance = norm(gaussian_means[i], uniAreaCenter);
    		}
		}
    	...
    	for(i=0; i<(guassian_sample_n + uniform_sample_n); i++)
        {
            ...
            for(j=0; j<N+1; j++)
            {
            	...
            	if(j!=cluster_label)probsMat.at<double>(i,j) = prob_2DGaussian(sample_i, gaussian_means[j], gaussian_cov[j]);
            	else probsMat.at<double>(i,j) = 1.0/uniform_area;
    	...
    	kmeans_img = Scalar(0);
    	rectangle(kmeans_img, Point(kmeans_img.rows/(N1+1), kmeans_img.rows/(N1+1)), Point(N1*kmeans_img.rows/(N1+1), N1*kmeans_img.rows/(N1+1)), Scalar(255, 255, 255));
    	...
    	for(i=0; i<N+1; i++)
        {
    		...
    		if(i!=cluster_label) draw_covMatrix_eclipse(kmeans_img, gaussian_cov[i], gaussian_means[i], 0);
        }

    	imshow( "uniform+GMM", kmeans_img );
    	waitKey(100);
    }
    ...
    imshow( "input points", img );
    imshow( "GMM", img_visual );
    imshow( "uniform+GMM", kmeans_img );
    waitKey(0);
    return 0;
