...
namespace golfcar_pcl{
	...
	void rolling_window_pcl::windowProcessing(ros::Time current_time)
	{
		...
		for(size_t i=0; i<windowXYZ_tmp.points.size();i++)
		{
          ...
          //----------NF3--------to focus on a fraction of the road surface-------------------------
			 //PointCloud patch_ROI;
          if(pointInPolygon(windowXYZ_tmp.points[i], poly_ROI_))
          {
	   ...
	   pcl::ModelCoefficients::Ptr coefficients (new pcl::ModelCoefficients);
		...
		//----------------------------to publish the plane coefficients--------------------;
		rolling_window::plane_coef segPlaneCoef;
		segPlaneCoef.header = rolling_window_baselink_.header;
		for(int i=0; i < 4; i++) segPlaneCoef.coefs.push_back(coefficients->values[i]);
		plane_coef_pub_.publish(segPlaneCoef);
		//----------------------------------------------------------------------------------
		
		//to visualize the segmented points;
	...
	void rolling_window_pcl::publishNormal(pcl::PointCloud<pcl::PointNormal>& pcl_cloud)
	{
		...
		{	
			if(fabs(pcl_cloud.points[i].curvature)>0.2)
			{
