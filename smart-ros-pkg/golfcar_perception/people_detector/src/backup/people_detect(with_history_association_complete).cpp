			    ...
			    for(std::vector<labeled_cluster>::size_type ic=0; ic!=processed_clusters_para.size(); ic++)  //one way: clusters try to find matching history;
			    {
					...
					if(dis2d<(1+history_pool[nearest_cluster].unreg_times)*0.1)   // (1)nearest history; (2)within 0.1 meter.
					{
						if(history_pool[nearest_cluster].reg_flag==false)   // avoid multiple registration for one cluster history.
						{
						   ...
						   //ROS_INFO("update history: distance: %5f, cluster: %d, history: %d",dis2d, ic, history_pool[nearest_cluster].object_label);
						}
					}
			  ...
			  for(std::vector<cluster_history>::size_type ih=0; ih!=history_pool.size(); ih++)  //two way: from the view of history.
				{
					...
					if(history_pool[ih].unreg_times==3)
					{
					    stale_numbers.push_back(ih);
					    ROS_INFO("*****stale history_pool: %d, object_label %d*****", ih, history_pool[ih].object_label);
					    //history_pool.erase(history_pool.begin()+ih);        //after erase, vector中的标号顺序也会改变……这里要改。问题很有可能出在这里。因为注释掉这一行以后，就不会出现segmentation fault的问题了。
					    //ROS_INFO("stale history deleted: %d", ih);
					} 
					...
					else
					{
						...
						if(moving_range>0.8&&moving_range<4)
						{
				...
				{
					//std::vector<int>::size_type it=stale_numbers.size()-1
					int it=stale_numbers.size()-1;
					ROS_INFO("stale numbers total number: %d",it);
					for(it=stale_numbers.size()-1; it>=0; it--)
					{
					 ROS_INFO("stale history deleted: %d, object %d",stale_numbers[it], history_pool[stale_numbers[it]].object_label);
