...
namespace people_detector{
	...
	people_detect::people_detect()
	{
		...
		//"temp_veri_obj_shows" stores parts of "veri_objs" recieving from "camera_project", in FOV of camera;
					...
					else
					{
						...
						if(history_pool[ih].mov_flag)
						{
						   ...
						   if(ped_point_temp.y>0)
						   {
								...
								{history_pool[ih].inFOV_flag=true;DisSickCoord=ped_point_temp.x;}
