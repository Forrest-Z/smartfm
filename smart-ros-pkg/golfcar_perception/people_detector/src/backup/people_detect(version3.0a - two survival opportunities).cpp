		   ...
		   else                          //association with history;
		   {
			    ...
			    {	
					...
					else                 //no matching history; to store it in temp_cluster_pool
					{
						...
						{
							if(history_pool[nearest_cluster].reg_flag==false)history_pool[nearest_cluster].its_possible_descendant=possible_cluster;  //"if" is very important here.
			  ...
			  for(std::vector<cluster_history>::size_type ih=0; ih!=history_pool.size(); ih++)  //two way: from the view of history.
				{
					...
					if(history_pool[ih].reg_flag==false)
					{
						...
						//the condition that need to be veryfied is checked in former part.---if(dis2d<(1+history_pool[nearest_cluster].unreg_times)*searching_range*4)
