	   ...
	   void people_detect::associateHistory(std::vector<labeled_cluster> &processed_clusters_para)
	   {
		   ...
		   else                          //association with history;
		   {
			    ...
			    {					
			        ...
			        if(history_pool[nearest_cluster].mov_flag==false){unreg_limit=4;searching_range=0.15;}
			        else {unreg_limit=4;searching_range=0.2;}
					...
					*/					
					...
					else                 //no matching history; to create one new piece of history.
					{
						...
						if(history_pool[nearest_cluster].reg_flag==false)//pay attention! ^_^
						{temp_history.its_possible_ancestor = history_pool[nearest_cluster].object_label;  //mark its possible ancestor;
			  ...
			  for(std::vector<cluster_history>::size_type ih=0; ih!=history_pool.size(); ih++)  //2nd view: from the view of history.
				{
					...
					if(history_pool[ih].unreg_times>0&&history_pool[ih].its_possible_descendant!=-1)
					{
						
						
						for(std::vector<cluster_history>::size_type ihh=0; ihh!=history_pool.size(); ihh++) 
						{
							if(history_pool[ihh].its_possible_ancestor==history_pool[ih].object_label)
							{descendant_serial=ihh;
							ROS_INFO("oooooooooo--------- descendant_serial %d --------ooooooooo", ihh);}
						}
						...
						if(dis2dtemp<(0+history_pool[ih].unreg_times)*searching_range)
						{history_pool[ih].reborn_label=true;} 
					}
				
					if(history_pool[ih].reborn_label==true)
					{
						ROS_INFO("OK HERE");
					    history_pool[ih].object_label=history_pool[descendant_serial].object_label;       //to avoid confusion of descendant's descendant
