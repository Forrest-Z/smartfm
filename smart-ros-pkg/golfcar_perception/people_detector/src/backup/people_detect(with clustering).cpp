			...
			{				
				...
				if(dis_x<4&&dis_x>-4&&dis_y<4&&dis_y>-4)
				{
					...
					if(ix==(extracted_points_para.size()-1))
					{
						...
						ROS_INFO("cluster_lianshangle: %d, point_number: %d", temp_cluster.label_num, temp_cluster.points_sum);
				...
				{
					extracted_clusters.push_back(temp_cluster);
					ROS_INFO("cluster: %d, point_number: %d", temp_cluster.label_num, temp_cluster.points_sum);
					...
					{
						extracted_clusters.push_back(temp_cluster);	
						ROS_INFO("cluster_meilianshang: %d, point_number: %d", temp_cluster.label_num, temp_cluster.points_sum);
						ROS_INFO("finished! total cluster: %d",temp_cluster.label_num);
					}			
				}
			}
