...
namespace people_detector{
	...
	people_detect::people_detect()
	{
		...
		ros::NodeHandle nh;
		map_sub_ = nh.subscribe("map", 1, &people_detect::initMap, this);  // change "map"sick_map: the second map.
  ...
  //	laser_sub = nh.subscribe<sensor_msgs::LaserScan>("scan", 1, boost::bind(&people_detect::scanCallback, this, _1));
	...
	{	
	    ...
	    //ROS_INFO("laser callback finished");
	    	    
	    people_detect::tfSickPose(sick_to_map);
	    //ROS_INFO("tfSickPose finished");
       ...
       void people_detect::extractClusters(const std::vector<labeled_point> &extracted_points_para)
      {
	   ...
	   void people_detect::associateHistory(std::vector<labeled_cluster> &processed_clusters_para)
	   {
			  ...
			  for(std::vector<cluster_history>::size_type ih=0; ih!=history_pool.size(); ih++)  //2nd view: from the view of history.
				{
					...
					else
					{
						...
						if(history_pool[ih].mov_flag)
						{
						   ...
						   {if(ped_point_temp.x>1.732*ped_point_temp.y) history_pool[ih].inFOV_flag=true;
				...
				// ROS_INFO("********************************Version 3.0b One Round of History Association Finished, History_pool Size: %d*************************", (int)history_pool.size());	
		    }
		}
		...
		{
			veri_pedestrians_cloud.points.clear();  //if not clear, path of these pedestrians will be showed.
		    veri_pedestrians_cloud.header.stamp=laser_cloud.header.stamp;
		    
		    int TrackedNum_temp=0;
		    int VerifiedNum_temp=0;
		    
		    people_detector::people_rects temp_veri_obj_shows;
		    
			for(std::vector<cluster_history>::size_type ih=0; ih!=history_pool.size(); ih++)
			{
				if(history_pool[ih].mov_flag==true)
				{
					if(history_pool[ih].inFOV_flag==true)
					{TrackedNum_temp++;}
					...
					{ 	
						...
						for(unsigned int i=0; i<veri_objs.pr_vector.size(); i++)
						{
							...
							temp_veri_obj_shows.pr_vector.push_back(temp_veri_obj_show);
						}
			...
			veri_obj_shows.publish(temp_veri_obj_shows);
