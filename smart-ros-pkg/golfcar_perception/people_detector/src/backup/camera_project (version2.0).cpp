...
namespace camera_projector{
	...
	void camera_project::project_to_image(const boost::shared_ptr<const sensor_msgs::PointCloud>& pedestrians_cloud_para) 
	{
		...
		if((temppr.x-(int)(0.5/temppr.disz*webcam.fc[0]))<0)
		{tempprcorner.x=0;}
		else
		{tempprcorner.x=temppr.x-(int)(0.5/temppr.disz*webcam.fc[0]);}
		
		//to get tempprcorner.width
		if(((int)(0.5/temppr.disz*webcam.fc[0])+temppr.x)>1280)
		{tempprcorner.width=1280-tempprcorner.x;}
		else
		{tempprcorner.width=(int)(0.5/temppr.disz*webcam.fc[0])+temppr.x-tempprcorner.x;}		 
		
		//to get tempprcorner.y
		if((temppr.y-(int)(1.2/temppr.disz*webcam.fc[0]))<0)
		{tempprcorner.y=0;}
		else
		{tempprcorner.y=temppr.y-(int)(1.2/temppr.disz*webcam.fc[0]);}
		
		//to get tempprcorner.height
		if(((int)(1.8/temppr.disz*webcam.fc[0])+temppr.y)>1024)
		{tempprcorner.height=1024-tempprcorner.y;}
		else
		{tempprcorner.height=(int)(1.8/temppr.disz*webcam.fc[0])+temppr.y-tempprcorner.y;}		 
