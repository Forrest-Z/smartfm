	...
	{	
       ...
       void people_detect::extractClusters(const std::vector<labeled_point> &extracted_points_para)
      {
	   ...
	   void people_detect::associateHistory(std::vector<labeled_cluster> &processed_clusters_para)
	   {
				...
				if(history_pool[ih].mov_flag && history_pool[ih].inFOV_flag)
				{
					...
					for(unsigned int i=0; i<veri_objs.pr_vector.size(); i++)
					{
						...
						if(history_pool[ih].object_label==veri_objs.pr_vector[i].object_label)
						{
								...
								people_detect::HistorCreat(temp_veri_obj_show, LaserDisHistor_);
								...
								{ 	
									...
									people_detect::HistorCreat(temp_veri_obj_show, VisionDisHistor_);
								}
						}	
					}
				}
		...
		void people_detect::HistorCreat(const people_detector::people_rect& temp_veri_obj_para, std::vector<unsigned int>& HistorPara)
		{
			...
			if(tempinteger>=HistorPara.size())
			{	
				unsigned int initialcount = 0;
				initialcount=1;
				HistorPara.push_back(initialcount);
			}
			else
			{
				HistorPara[tempinteger]=HistorPara[tempinteger]+1;
			}
		}
