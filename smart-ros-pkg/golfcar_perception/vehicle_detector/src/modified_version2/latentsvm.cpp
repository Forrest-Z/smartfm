...
CvLSVMFeaturePyramid* createFeaturePyramidWithBorder(IplImage *image,
                                               int maxXBorder, int maxYBorder, int long_side, int &starting_level)
{
    int opResult;
    int bx, by;
    int level;
    ...
    opResult = getFeaturePyramid(image, &H, long_side, starting_level);
    ...
    //modify
    if(!REDUCE_FEATURE_PYRAMID) level = 0;
    else
    {
		if(starting_level==-1)level = H->numLevels;
		else if( starting_level == LAMBDA) level = 0;
		else level = starting_level - LAMBDA;
	}
	
    //printf("\n  H->numLevels, starting_level, level: %d, %d, %d \n", H->numLevels, starting_level, level);
    for (; level < H->numLevels; level++)
    {
		//printf("level: %d \t", level);
        addNullableBorder(H->pyramid[level], bx, by);
    }
    //printf("createFuture finish! \n");
    ...
    *kPoints = 0;
    ...
    for (i = 0; i < kComponents; i++)
    {
        int error = searchObjectThreshold(H, &(filters[componentIndex]), kPartFilters[i],
            b[i], maxXBorder, maxYBorder, scoreThreshold, starting_level,
