...
CvSeq* cvLatentSvmDetectObjects(IplImage* image,
                                CvLatentSvmDetector* detector,
                                CvMemStorage* storage, int long_side,
                                float overlap_threshold, int numThreads)
{
    ...
    printf("max filter size %d, %d\n", maxXBorder, maxYBorder);
    
    // Create feature pyramid with nullable border
	clock_t time_begin1, time_begin2;
	clock_t delt_t1, delt_t2; 
	
	time_begin1 = clock();
	int starting_level;
    H = createFeaturePyramidWithBorder(image, maxXBorder, maxYBorder, long_side, starting_level);
    delt_t1 = clock() - time_begin1;
    
    // Search object
    time_begin2 = clock();
    error = searchObjectThresholdSomeComponents(H, (const CvLSVMFilterObject**)(detector->filters),
        detector->num_components, detector->num_part_filters, detector->b, detector->score_threshold, starting_level,
        &points, &oppPoints, &score, &kPoints, numThreads);
    if (error != LATENT_SVM_OK)
    {
        return NULL;
    }
    delt_t2 = clock() - time_begin2;
    
    printf ("feature calculation: %f s; search: %f s", (float)delt_t1/CLOCKS_PER_SEC, (float)delt_t2/CLOCKS_PER_SEC );
    ...
    freeFeaturePyramidObject(&H, starting_level);
    free(points);
    free(oppPoints);
    free(score);
                                ...
                                int numThreads )
{
    ...
    for( size_t classID = 0; classID < detectors.size(); classID++ )
    {
        ...
        CvSeq* detections = cvLatentSvmDetectObjects( &image_ipl, detectors[classID], storage, long_side, overlapThreshold, numThreads);
