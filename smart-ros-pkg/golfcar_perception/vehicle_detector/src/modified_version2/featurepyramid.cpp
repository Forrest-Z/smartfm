...
int getFeaturePyramid(IplImage * image, CvLSVMFeaturePyramid **maps, int long_side, int &starting_level)
{
	...
	starting_level = 0;
	if(longsideNumStep - LAMBDA> numStep)
	{
		starting_level = -1;
	}
	else
	{
		if(longsideNumStep >= 2*LAMBDA)
		{
			starting_level = (longsideNumStep - LAMBDA) + LAMBDA;
		}
		else
		{
			starting_level = LAMBDA;
		}
	}

	printf("numStep, longsideNumStep, starting_level, %d, %d, %d\n", numStep, longsideNumStep, starting_level);
	...
	{	
		...
		{	
			...
			getPathOfFeaturePyramid(imgResize, step, 2*LAMBDA, numStep+LAMBDA-2*LAMBDA, SIDE_LENGTH, maps);
		}
		else
		{
		...
		*/ 
		...
		if(starting_level != -1)
		{
			if(starting_level == longsideNumStep)
			{
