...
vehicle_detection::vehicle_detection(ros::NodeHandle &n) : private_nh_("~"), n_(n), it_(n_)
{
	...
	private_nh_.param("camera_frame_id",    camera_frame_id_,     std::string("camera_front"));
...
void vehicle_detection::lidarMeas_callback(const MODT::segment_pose_batches& batches)
{
	...
	cluster_centroids.header = batches.header.frame_id;
	cluster_centroids.header.stamp = cv_image_->header.frame_id;
	...
	for(size_t i=0; i<cluster_centroids.points.size(); i++)
	{
		...
		try{tf_.transformPointCloud(camera_frame_id_, right_lower, right_lower);}
		catch (tf::TransformException& e){ROS_DEBUG("Wrong!!!!!!!!!!!!!"); std::cout << e.what();continue;}
		try{tf_.transformPointCloud(camera_frame_id_, left_upper, left_upper);}
		catch (tf::TransformException& e){ROS_DEBUG("Wrong!!!!!!!!!!!!!"); std::cout << e.what();continue;}

		cv::Point3d right_lower_cv(right_lower.x(), right_lower.y(), right_lower.z());
		cv::Point2d right_lower_uv;
		cam_model_.project3dToPixel(right_lower_cv, right_lower_uv);
		cv::Point3d left_upper_cv(left_upper.x(), left_upper.y(), left_upper.z());
		cv::Point2d left_upper_uv;
		cam_model_.project3dToPixel(left_upper_cv, left_upper_uv);

		Rect roi(left_upper_uv.x, left_upper_uv.y, right_lower_uv.x-left_upper_uv.x, right_lower_uv.y-left_upper_uv.y);
	...
	Mat imageROI (mat_img, image_ROIs.front());
