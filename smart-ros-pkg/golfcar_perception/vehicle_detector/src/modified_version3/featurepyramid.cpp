                            ...
                            int sideLength, CvLSVMFeaturePyramid **maps)
{
    ...
    for(i = 0; i < numStep; i++)
    {
        ...
        if(startIndex<LAMBDA)scale = 1.0f / powf(step, (float)(i+startIndex));
...
// Error status
*/
int getFeaturePyramid(IplImage * image, CvLSVMFeaturePyramid **maps, int long_side, int &starting_level)
{
    IplImage *imgResize;
    float step;
    int   numStep;
    ...
    int longsideNumCell = long_side/SIDE_LENGTH;
    int longsideNumStep = (int)(logf((float) longsideNumCell / (5.0f)) / logf( step )) + 1;


	//starting_level uses to denotes from which level to search for the objects;
	starting_level = 0;
	if(longsideNumStep - SEARCH_LEVELS > numStep)
	{
		starting_level = -1;
	}
	else
	{
		starting_level = (longsideNumStep - SEARCH_LEVELS) + LAMBDA;
		if(starting_level<LAMBDA) starting_level = LAMBDA;
	}
	
	int begin_level = starting_level-LAMBDA ;
	...
	{	
		if(starting_level != -1)
		{		
			//begin_level to calculate the pyramid features, which is LAMBDA level (part-level) down than the body-level to search;
			if(begin_level >= LAMBDA)
			{
				getPathOfFeaturePyramid(imgResize, step, numStep + LAMBDA - begin_level, begin_level, SIDE_LENGTH, maps);
			}
			else
			{
				getPathOfFeaturePyramid(imgResize, step, LAMBDA-begin_level, begin_level, SIDE_LENGTH / 2, maps);
				getPathOfFeaturePyramid(imgResize, step, numStep, LAMBDA, SIDE_LENGTH, maps);
			}			
		}
	}
	
	printf("numStep, longsideNumStep, starting_level, begin_level: %d, %d, %d, %d\n", numStep, longsideNumStep, starting_level, begin_level);        
