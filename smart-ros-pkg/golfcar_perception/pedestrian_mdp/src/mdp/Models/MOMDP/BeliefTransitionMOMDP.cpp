...
{
	cout << "using this belief update belY, belX, act, obs, obsX .. tirtha " << endl;
	...
	if (!((momdpProblem->obsProb->getMatrix(a, xn))->isColumnEmpty(o))) 
	{
		...
		{
			cout << "In nextBelief(), the combination of b, a, o and x' is not possible." << endl;
			throw runtime_error("In nextBelief(), the combination of b, a, o and x' is not possible.");
		}
...
{
	cout << " nextBelief beliefState, act, obs, obsX  " << endl;
	SparseVector jspv;
	SharedPointer<MOMDP> momdpProblem = dynamic_pointer_cast<MOMDP> (problem);
...
{
	//cout << " nextBelief 2, beliefState bp, act, obs, obsX, jspv " << endl;
	SharedPointer<MOMDP> momdpProblem = dynamic_pointer_cast<MOMDP> (problem);
	//cout << " jspv " ;// << endl;
	//jspv->write(cout) << endl;
