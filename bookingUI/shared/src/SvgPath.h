...
public:
    ...
    SvgPath(const char* pFilename, StationPath* pose, PathPoint *size);
    void convert_to_meter(StationPath* pose, PathPoint &size, double resolution);
    
private:
    unsigned int foundPath;
    void findPathElements(StationPath* pose, PathPoint* size, unsigned int *npoints, TiXmlNode* pParent);
    ...
    int find_path_attributes(TiXmlElement* pElement, StationPath* pose);
